...
class Play_screen(Frame):
    ...
    def letter_click(self,letter,list, string):
        ...
        if self.totalcount == 1:
            # GO TO TURTLEHANGMAN.PY AND FINISH WRITING THE FUNCTIONS. WHAT DEF LETTER_CLICK DOES IS IT TAKES THE LETTER
            # THAT THE USER CHOSE AND COMPARES EVERY ITEM IN THE GUESS_LIST (GUESS_LIST CONTAINS ALL THE LETTERS OF THE WORD THEY HAVE TO GUESS)
            # TO THE LETTER THAT THEY CHOSE. IF THE LETTER ISN'T EQUAL TO ONE OF THE LETTERS IN THE LIST, THEN IT TAKES A STRING AND ADDS A BLANK TO IT ("_")
            # IF THE LETTER IS EQUAL TO TO ONE OF THE LETTERS IN THE LIST, IT ADDS THAT LETTER TO THE STRING. IT ALSO MAKES SELF.TOTALCOUNT = 0
            # NOTE THAT SELF.TOTALCOUNT EQUALS ONE AT FIRST. IF AT THE END OF THE ENTIRE FOR LOOP, SELF.TOTALCOUNT ISN'T EQUAL TO 0, THAT MEANS THAT
            # THE USER'S GUESS IS WRONG. IF IT IS, THEN IT GOES TO THE STATEMENT "IF SELF.TOTALCOUNT == 1:"
